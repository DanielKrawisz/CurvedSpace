...
func NewPlaneByPointAndNormal(point, norm []float64, right_side_out bool) surface.Surface {
	if point == nil || norm == nil {
		return nil
	}
	if len(point) != len(norm) {
		return nil
	}

	if vector.Dot(norm, norm) == 0 {
		return nil
	}
	if right_side_out {
...
func (s *linearSurface) Translate(v []float64) surface.Surface {
	p := vector.Negative(v)
...
//
func (s *quadraticSurface) Translate(v []float64) surface.Surface {
	p := vector.Negative(v)
...
//
func (s *cubicSurface) Translate(v []float64) surface.Surface {
	p := vector.Negative(v)
...
//
func (s *quarticSurface) Translate(v []float64) surface.Surface {
	p := vector.Negative(v)
...
func (s *linearSurface) CoordinateShift(A [][]float64) surface.Surface {
	s.b = vector.MatrixMultiply(A, s.b)
	return s
}

func (s *quadraticSurface) CoordinateShift(A [][]float64) surface.Surface {
	s.b = vector.MatrixMultiply(A, s.b)
	s.c = vector.MatrixMultiplySymmetricTensor(A, s.c)
	return s
}

func (s *cubicSurface) CoordinateShift(A [][]float64) surface.Surface {
	s.b = vector.MatrixMultiply(A, s.b)
	s.c = vector.MatrixMultiplySymmetricTensor(A, s.c)
...
func (s *quarticSurface) CoordinateShift(A [][]float64) surface.Surface {
	s.b = vector.MatrixMultiply(A, s.b)
	s.c = vector.MatrixMultiplySymmetricTensor(A, s.c)
	s.d = vector.MatrixMultiplySymmetric3Tensor(A, s.d)
...
func NewQuadraticSurface(p []float64, vp, vn [][]float64, y []float64, r2 float64) surface.Surface {
	...
	return (&quadraticSurface{dim, c, vector.Negative(y), r2}).Translate(p)
}

func NewQuadraticSurfaceByTensorParameters(c [][]float64, b []float64, a float64) surface.Surface {
	dim := len(c)
	if dim != len(b) {
		return nil
	}
...
func NewCubicSurface(p []float64, vd, vp, vn [][]float64, y []float64, r3 float64) surface.Surface {
	...
	return (&cubicSurface{dim, d, c, vector.Negative(y), r3}).Translate(p)
}
...
func NewQuarticSurface(p []float64, vqp, vqn, vd, vp, vn [][]float64, y []float64, r4 float64) surface.Surface {
	...
	return (&quarticSurface{dim, e, d, c, vector.Negative(y), r4}).Translate(p)
}
