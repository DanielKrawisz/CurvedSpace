...
func TestRungeKuttaObjects(t *testing.T) {
	...
	if rkmethods == nil {
		t.Error("Why isn't this working??")
	}
	...
	for i := 0; i < len(rkmethods); i++ {
		if rkmethods[i] == nil {
			...
			if rkmethods[i].Name() != rknames[i] {
				t.Error("Wrong RK method loaded")
			}
		}
	}
}
...
func TestEulerAgainstEuler(t *testing.T) {
	...
	for i, K := range Klist {
		...
		spring1 := NewHarmonicOscillator(particle1, []float64{1, 2}, [][][]float64{nil, {K}})
		spring2 := NewHarmonicOscillator(particle2, []float64{1, 2}, [][][]float64{nil, {K}})
...
func TestRungeKuttaHarmonicOscillatorNoStepSizing(t *testing.T) {
	...
	for i := 0; i < len(rkmethods); i++ {
		...
		for _, stepIncrement := range stepIncrements {
			...
			spring := NewHarmonicOscillator(particle, []float64{1, 2}, [][][]float64{nil, {{1}}})
...
func TestRungeKuttaHarmonicOscillatorNonlinearNoStepSizing(t *testing.T) {
	...
	for i := 0; i < len(rkmethods); i++ {
		...
		spring := NewHarmonicOscillator(particle, []float64{1, 1}, [][][]float64{nil, {{1, .2}}})
